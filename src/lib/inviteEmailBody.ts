...
export function buildAssessmentEntriesFromTests(
  tests: Array<{
    id: string
    testType: TestType
    durationMinutes: number
    questions: unknown[]
    personalityQuestions?: unknown[]
  }>,
  origin: string,
): AssessmentInviteEntry[] {
  return tests.map((test) => ({
    testType: test.testType ?? 'technical',
    label: TEST_TYPE_EMAIL_LABELS[test.testType ?? 'technical'],
    durationMinutes: test.durationMinutes,
    timed: (test.testType ?? 'technical') === 'technical',
    questionCount:
      test.testType === 'personality'
        ? (test.personalityQuestions?.length ?? 0)
        : test.questions.length,
    url: `${origin}/test/${test.id}`,
  }))
}
